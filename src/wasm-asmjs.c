...
#define PROGRAM_NAME "wasm-asmjs"
...
static WasmBinaryErrorHandler s_binary_error_handler =
    WASM_BINARY_ERROR_HANDLER_DEFAULT;
static WasmSourceErrorHandler s_source_error_handler =
...
static const char s_description[] =
    "  read a file in the wasm binary format, and convert it to asm.js\n"
    "\n"
    "examples:\n"
    "  # parse binary file test.wasm and write asm.js file test.js\n"
    "  $ wasm-asmjs test.wasm -o test.js\n";
...
int main(int argc, char** argv) {
  ...
  if (WASM_SUCCEEDED(result)) {
    ...
    result = wasm_read_binary_ast(allocator, data, size, &s_read_binary_options,
                                  &s_binary_error_handler, &module);
    if (WASM_SUCCEEDED(result))
      result = wasm_check_ast_module(NULL, &module, &s_source_error_handler);

    if (WASM_SUCCEEDED(result))
      result = wasm_lower_expressions(allocator, &module);
    ...
    if (WASM_SUCCEEDED(result)) {
      ...
      if (WASM_SUCCEEDED(result)) {
        WasmWriteAsmjsOptions options = WASM_WRITE_ASMJS_OPTIONS_DEFAULT;
        result =
            wasm_write_asmjs(allocator, &file_writer.base, &module, &options);
